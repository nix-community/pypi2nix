...
def main(
    ...
    command = "{nix_shell} {nix_file} {options} {nix_path} -K --show-trace --pure --run exit".format(  # noqa
        ...
        options=pypi2nix.utils.create_command_options(
            dict(
        ...
        nix_path=nix_path and " ".join("-I {}".format(i) for i in nix_path) or "",
    )
    ...
    return (
        buildout_file and os.path.join(project_dir, "buildout_requirements.txt") or None
    )
