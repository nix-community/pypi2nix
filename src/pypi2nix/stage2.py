...
def process_metadata(wheel_dir, default_environment):
    ...
    raise click.ClickException("Unable to find METADATA in `%s` folder." % wheel_dir)
...
def find_release(wheel_cache_dir, wheel, wheel_data):
    ...
    if digests:
        ...
        filename = os.path.join(wheel_cache_dir, wheel_release["filename"])
...
def process_wheel(
    wheel_cache_dir, wheel, sources, verbose, index=INDEX_URL, chunk_size=2048
):
    """
    """
