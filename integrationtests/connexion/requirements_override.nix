...
self: super: {
  "jsonschema" = python.overrideDerivation super."jsonschema"
    (old: { buildInputs = old.buildInputs ++ [ self."vcversioner" ]; });
  ...
  "connexion" = super."connexion".overrideDerivation
    (old: { nativeBuildInputs = old.nativeBuildInputs ++ [ self."flake8" ]; });
}
