...
#   bin/pypi2nix -n plone43Packages -d Plone -d mailinglogger -d zc.recipe.egg -d plone.recipe.zope2instance -d Pillow -i setuptools -i zc_buildout -i lxml -i pillow -e plone/43.json -p plone/43.txt -o plone/43.nix

{ pkgs, pythonPackages }:

let plone43Packages = pythonPackages.python.modules // rec {
  inherit (pythonPackages) buildPythonPackage setuptools zc_buildout lxml pillow;
